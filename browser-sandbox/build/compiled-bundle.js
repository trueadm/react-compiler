(function (factory) {
  ...
  var isUnitlessNumber = new Set(["animation-iteration-count", "border-image-outset", "border-image-slice", "border-image-width", "box-flex", "box-flex-group", "box-ordinal-group", "column-count", "columns", "flex", "flex-grow", "flex-positive", "flex-shrink", "flex-negative", "flex-order", "grid-area", "grid-row", "grid-row-end", "grid-row-span", "grid-row-start", "grid-column", "grid-column-end", "grid-column-span", "grid-column-start", "font-weight", "line-clamp", "line-height", "opacity", "order", "orphans", "tab-size", "widows", "z-index", "zoom", // SVG-related properties
  ...
  var CONDITIONAL = 6;
  var LOGICAL = 7;
  var TEMPLATE_FUNCTION_CALL = 8;
  var MULTI_CONDITIONAL = 9;
  var TEXT_ARRAY = 10;
  var VNODE_ARRAY = 11;
  var REFERENCE_COMPONENT = 12;
  var REFERENCE_REACT_NODE = 13;
  var MULTI_RETURN_CONDITIONAL = 14;
  var CONTEXT_PROVIDER = 16;
  var CONTEXT_CONSUMER = 17; // Elements/Components/Context

  var HAS_STATIC_PROPS = 1 << 6;
  var HAS_DYNAMIC_PROPS = 1 << 7;
  var HAS_STATIC_STYLES = 1 << 8;
  var HAS_DYNAMIC_STYLES = 1 << 9;
  var HAS_CHILD = 1 << 10;
  ...
  function mountPropValue(DOMNode, propFlags, propName, propValue) {
    ...
    } else if ((propFlags & PROP_IS_EVENT) !== 0) ; else if ((propFlags & PROP_IS_BOOLEAN) !== 0) {
  ...
  function mountHostComponentTemplate(templateTypeAndFlags, hostComponentTemplate, parentDOMNode, values, currentFiber) {
    ...
    if ((templateFlags & HAS_STATIC_STYLES) !== 0) {
      ...
      var staticStyles = hostComponentTemplate[childrenTemplateIndex++];

      for (var _i = 0, _length = staticStyles.length; _i < _length; _i += 2) {
        var styleName = staticStyles[_i];
        var staticStyleValue = staticStyles[_i + 1];
        DOMNodeStyle.setProperty(styleName, staticStyleValue);
      }
    }

    if ((templateFlags & HAS_DYNAMIC_PROPS) !== 0) {
      var dynamicProps = hostComponentTemplate[childrenTemplateIndex++];

      for (var _i2 = 0, _length2 = dynamicProps.length; _i2 < _length2; _i2 += 3) {
        var _propName = dynamicProps[_i2];
        var dynamicPropFlags = dynamicProps[_i2 + 1];
        var dynamicPropValueIndex = dynamicProps[_i2 + 2];
        var dynamicPropValue = values[dynamicPropValueIndex];

        if (dynamicPropValue === undefined || dynamicPropValue === null) {
          continue;
        }

        mountPropValue(DOMNode, dynamicPropFlags, _propName, dynamicPropValue);
      }
    }
    ...
    if ((templateFlags & HAS_DYNAMIC_STYLES) !== 0) {
      ...
      var dynamicStyles = hostComponentTemplate[childrenTemplateIndex++];

      for (var _i3 = 0, _length3 = dynamicStyles.length; _i3 < _length3; _i3 += 2) {
        var _styleName = dynamicStyles[_i3];
        var dynamicStyleValueIndex = dynamicStyles[_i3 + 1];
        var dynamicStyleValue = values[dynamicStyleValueIndex];
        ...
        if (typeof dynamicStyleValue === "number" && dynamicStyleValue !== 0 && !isUnitlessNumber.has(_styleName)) {
          dynamicStyleValue = dynamicStyleValue + "px";
        }

        DOMNodeStyle.setProperty(_styleName, dynamicStyleValue);
      }
    }
  ...
  function mountMultiConditionalTemplate(multiConditionalTemplate, parentDOMNode, values, currentFiber) {
    ...
    var conditionalFiber = createFiber(multiConditionalTemplate, values);
    ...
    for (var i = 0, length = conditions.length; i < length; i += 2) {
      ...
      if (conditionValueIndexOrNull === null) {
        ...
        var hostNode = mountTemplateNode(conditionTemplateNode, parentDOMNode, values, conditionalFiber);
      ...
      if (conditionValue) {
        ...
        var _hostNode = mountTemplateNode(_conditionTemplateNode, parentDOMNode, values, conditionalFiber);

        conditionalFiber.hostNode = _hostNode;
        return _hostNode;
      }
    }
  }

  function mountReferenceComponentTemplate(templateTypeAndFlags, referenceComponentTemplate, parentDOMNode, values, currentFiber) {
    var templateFlags = templateTypeAndFlags & ~0x3f;
    var componentTemplateNodeOrFunc = referenceComponentTemplate[1];
    var componentTemplateNode = componentTemplateNodeOrFunc;
    ...
    if ((templateFlags & HAS_STATIC_PROPS) !== 0) {
      ...
      var propsValueIndex = referenceComponentTemplate[2];
  ...
  function mountMultiReturnConditionalTemplate(multiReturnTemplateNode, parentDOMNode, values, currentFiber) {
    var conditionBranchToUseKey = values[0];
    var conditionalFiber = createFiber(multiReturnTemplateNode, values);
    insertFiber(currentFiber, conditionalFiber);

    for (var i = 1; i < multiReturnTemplateNode.length; i += 2) {
      var branchIndex = multiReturnTemplateNode[i];

      if (branchIndex === conditionBranchToUseKey) {
        pushSlotValue(currentFiber, i);
        var branchTemplateNode = multiReturnTemplateNode[i + 1];
  ...
  function mountReferenceReactNodeTemplate(referenceReactNodeTemplateNode, parentDOMNode, values, currentFiber) {
    var reactNodeValueIndex = referenceReactNodeTemplateNode[1];
    var reactNode = values[reactNodeValueIndex];
  ...
  function mountReactNode(reactNode, parentDOMNode, currentFiber) {
    if (reactNode === undefined || reactNode === null || typeof reactNode === "boolean") {
      var reactNodeFiber = createFiber(null, reactNode);
      insertFiber(currentFiber, reactNodeFiber);
      var hostNode = createPlaceholder();
    ...
    if (typeof reactNode === "string" || typeof reactNode === "number") {
      var _reactNodeFiber = createFiber(null, reactNode);

      insertFiber(currentFiber, _reactNodeFiber);

      var _hostNode2 = createTextNode(reactNode);

      _reactNodeFiber.hostNode = _hostNode2;

      if (parentDOMNode !== null) {
        appendChild(parentDOMNode, _hostNode2);
      }
    ...
    if (isArray(reactNode)) {
      var vNodeArrayFiber = createFiber(null, reactNode);
      insertFiber(currentFiber, vNodeArrayFiber);
      var hostNodes = [];

      for (var i = 0, length = reactNode.length; i < length; ++i) {
  ...
  function mountVNode(vNode, parentDOMNode, currentFiber) {
    var vNodeFiber = createFiber(vNode.t, vNode.v);
    insertFiber(currentFiber, vNodeFiber);
    vNode.f = vNodeFiber;
    var templateNode = vNode.t;
    var values = vNode.v;

    var hostNode = mountTemplateNode(templateNode, parentDOMNode, values, vNodeFiber);
    vNodeFiber.hostNode = hostNode;
    return hostNode;
  }

  function mountConditionalTemplate(conditionalTemplate, parentDOMNode, values, currentFiber) {
    var conditionalValueIndex = conditionalTemplate[1];
    var conditionalFiber = createFiber(conditionalTemplate, values);
    insertFiber(currentFiber, conditionalFiber);
    var conditionalValue = values[conditionalValueIndex];
    var hostNode;

    if (conditionalValue) {
      var consequentTemplate = conditionalTemplate[2];
      ...
      var alternateTemplate = conditionalTemplate[3];
  ...
  function mountTemplateNode(templateNode, parentDOMNode, values, currentFiber) {
    ...
    switch (templateType) {
      ...
      case COMPONENT:
        return mountFunctionComponentTemplate(false, templateTypeAndFlags, templateNode, parentDOMNode, values, currentFiber);
